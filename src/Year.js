...
class YearView extends React.Component {
  ...
  render() {
    let { date, className } = this.props,
      beginOfYear = dates.startOf(date, 'year'),
      months = this.returnYearMonths(beginOfYear)
    return (
      <div className={clsx('rbc-year-view', className)}>
        {months.map(this.renderMonth)}
      </div>
    )
  }

  returnYearMonths = beginOfYear => {
    let months = [],
      currentMonth = dates.add(beginOfYear, 1, 'minutes')
    for (let i = 0; i <= 11; i++) {
      let monthDays = dates.range(
        dates.startOf(currentMonth, 'month'),
        dates.endOf(currentMonth, 'month')
      )
      months.push(monthDays)
      currentMonth = dates.add(currentMonth, 1, 'month')
    }
    return months
  }

  renderMonth = (month, id) => {
    let { className, localizer } = this.props,
      weeks = chunk(month, 7)
    this._weekCount = weeks.length
    return (
      <div className={clsx('rbc-month-view', className)} key={id}>
        <div className="rbc-month-title">
          {localizer.format(month[0], 'MMMM')}
        </div>
